...
namespace QuanLyKhachSan.DAO
{
    public class RoomDAO
    {
        ...
        public DataTable GetListOrderedRoomOfClient(string username)
        {
            ...
            string query = string.Format("SELECT p.MaPhong, p.TenPhong, p.GhiChu, kh.TenKhachHang FROM PHONG p, PHIEUTHUEPHONG ptp, KHACHHANG kh WHERE ptp.MaKH = kh.MaKH AND ptp.MaPhong = p.MaPhong AND kh.CMND = '{0}'", username);
